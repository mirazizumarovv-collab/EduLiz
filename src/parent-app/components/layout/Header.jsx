...
export function Header({ onOpenNotifications, hasUnreadNotifications, onOpenSwitcher, onBack, showBack }) {
  ...
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px", borderBottom: `1px solid ${c.border}` }}>
      {showBack ? (
        <button onClick={onBack} aria-label={t("back")} style={{ display: "flex", alignItems: "center", gap: 4, border: "none", background: "transparent", cursor: "pointer", padding: 4, color: c.textPrimary }}>
          <ArrowLeft size={20} />
        </button>
      ) : (
